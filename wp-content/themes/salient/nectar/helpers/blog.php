 ...
 */
 ...
 if( !function_exists('nectar_estimated_reading_time') ) {
  function nectar_estimated_reading_time( $content = '') {
	 ...
	 // Match and remove the shortcode wrappers but keep the text inside
	 $pattern = '/\[(\/?[\w-]+)([^\]]*)\](.*?)\[\/\1\]/s';
	 $content = preg_replace_callback($pattern, function($matches) {
		 // Return the inner text of the shortcode
		 return $matches[3];
	 }, $content);
 
	 // Now remove any self-closing shortcodes that may still be left
	 $content = preg_replace('/\[\/?[\w-]+[^\]]*\]/', '', $content);
 
    $text_only = strip_tags( trim($content) );
